...
import styles from './Review.table.less'
...
class ReviewTable extends PureComponent {
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ReviewBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/review/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
