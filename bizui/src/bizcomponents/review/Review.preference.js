...
import styles from './Review.preference.less'
...
const internalImageListOf = (review) =>{
...
const internalSettingListOf = (review) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ReviewService[candidateServiceName] 
...
const executeTrans = (review,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_review/doJob",payload: payload})
...
const buildTransferModal = (review,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(review,targetComponent)}
...
const internalRenderExtraHeader = (review) =>{
	return null
}
const internalRenderExtraFooter = (review) =>{
	return null
}
...
<Description term="User">{review.user==null?appLocaleName(userContext,"NotAssigned"):review.user.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User","profile","requestCandidateUser",
	      "transferToAnotherUser","anotherUserId",review.user?review.user.id:"")} 
...
<Description term="Product">{review.product==null?appLocaleName(userContext,"NotAssigned"):review.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Product","product","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",review.product?review.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = review => {
  const {ReviewBase} = GlobalComponents
  return <PermissionSetting targetObject={review}  targetObjectMeta={ReviewBase}/>


}


class ReviewPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Review",cardsFor: "review",cardsSource: this.props.review,
  		subItems: [
