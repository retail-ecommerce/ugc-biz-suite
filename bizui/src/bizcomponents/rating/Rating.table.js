...
import styles from './Rating.table.less'
...
class RatingTable extends PureComponent {
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = RatingBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/rating/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
