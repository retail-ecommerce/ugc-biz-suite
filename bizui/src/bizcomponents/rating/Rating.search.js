...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{ratingList:selectedRows}} owner={owner}  
...
class RatingSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {RatingTable} = GlobalComponents;
    const {RatingSearchForm} = GlobalComponents;
    const {RatingModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        ...
        {showDeletionDialog(this,RatingModalTable,"ratingIds")}
...
export default Form.create()(RatingSearch)
