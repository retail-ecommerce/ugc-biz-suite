...
import styles from './Rating.updateform.less'
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const ratingId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, ratingId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextRatingUpdateRow`,
        payload: {
          id: owner.id,
          type: 'rating',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新Rating"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新Rating"
