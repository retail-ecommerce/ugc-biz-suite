...
export default {
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._rating)
      ...
      const data = yield call(RatingService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {RatingService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(RatingService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/rating/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/rating/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/rating/${id}/list/${type}List/${listName}`))
    },
