...
import styles from './Blog.preference.less'
...
const internalImageListOf = (blog) =>{
...
const internalSettingListOf = (blog) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BlogService[candidateServiceName] 
...
const executeTrans = (blog,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_blog/doJob",payload: payload})
...
const buildTransferModal = (blog,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(blog,targetComponent)}
...
const internalRenderExtraHeader = (blog) =>{
	return null
}
const internalRenderExtraFooter = (blog) =>{
	return null
}
...
<Description term="User">{blog.user==null?appLocaleName(userContext,"NotAssigned"):blog.user.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"User","profile","requestCandidateUser",
	      "transferToAnotherUser","anotherUserId",blog.user?blog.user.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="Product">{blog.product==null?appLocaleName(userContext,"NotAssigned"):blog.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Product","product","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",blog.product?blog.product.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = blog => {
  const {BlogBase} = GlobalComponents
  return <PermissionSetting targetObject={blog}  targetObjectMeta={BlogBase}/>


}


class BlogPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Blog",cardsFor: "blog",cardsSource: this.props.blog,
  		subItems: [
