...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{blogList:selectedRows}} owner={owner}  
...
class BlogSearch extends PureComponent {
  ...
  render(){
    ...
    const {BlogTable} = GlobalComponents;
    const {BlogSearchForm} = GlobalComponents;
    const {BlogModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        ...
        {showDeletionDialog(this,BlogModalTable,"blogIds")}
...
export default Form.create()(BlogSearch)
