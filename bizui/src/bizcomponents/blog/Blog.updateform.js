...
import styles from './Blog.updateform.less'
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const blogId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, blogId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBlogUpdateRow`,
        payload: {
          id: owner.id,
          type: 'blog',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新Blog"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新Blog"
